  ...
  return `
    <article class="brew-recipe-card">
      ...
      <footer class="brew-recipe-footer">
        ...
        ${method.supportsDialIn ? `<button class="secondary-button button-with-icon" type="button" data-open-dial-in="${recipe.id}">${iconMarkup("dial-in", { group: "navigation" })}<span>${bean?.isGround ? "Brew log" : "Dial in"}</span></button>` : ""}
      </footer>
    </article>`;
}
